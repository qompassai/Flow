...
from flow.json_types import is_array, is_object, object_list, object_map
...
def validate_arguments(args: object, spec: Mapping[str, object]) -> None:
    """Validate Flow's bounded schema subset without trusting nested input types."""
    pending: list[tuple[object, Mapping[str, object], int]] = [(args, spec, 0)]
    visited = 0
    while pending:
        visited += 1
        if visited > SCHEMA_NODES_MAX:
            raise ValueError(f"Arguments exceed {SCHEMA_NODES_MAX} schema nodes")
        value, node_schema, depth = pending.pop()
        if depth > SCHEMA_DEPTH_MAX:
            raise ValueError(f"Arguments exceed nesting depth {SCHEMA_DEPTH_MAX}")
        _validate_node(value, node_schema)
        children: list[tuple[object, Mapping[str, object], int]] = []
        if is_object(value):
            properties = object_map(node_schema.get("properties", {}))
            if len(value) > SCHEMA_NODES_MAX - visited - len(pending):
                raise ValueError(f"Arguments exceed {SCHEMA_NODES_MAX} schema nodes")
            children = [
                (item, object_map(properties[key]), depth + 1) for key, item in value.items()
            ]
        elif is_array(value) and "items" in node_schema:
            items = object_map(node_schema["items"])
            if len(value) > SCHEMA_NODES_MAX - visited - len(pending):
                raise ValueError(f"Arguments exceed {SCHEMA_NODES_MAX} schema nodes")
            children = [(item, items, depth + 1) for item in value]
        pending.extend(children)


def _validate_node(value: object, schema: Mapping[str, object]) -> None:
    """Validate one node; unsupported schema types fail closed."""
    ...
    if "enum" in schema and value not in object_list(schema["enum"]):
        raise ValueError("Argument is not one of the allowed values")
    if is_object(value):
        properties = object_map(schema.get("properties", {}))
        required = object_list(schema.get("required", []))
